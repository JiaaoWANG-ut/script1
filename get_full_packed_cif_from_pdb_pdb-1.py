...
def packmol_to_zeo(input_file_zeo,input_file_mol,input_file_ion,number):
...
import numpy as np
def get_100_packing_number(input_file_zeo,input_file_mol,input_file_ion):
    for i in np.arange(0,1000,2):
        import time
        start=time.time()
        packmol_to_zeo(input_file_zeo,input_file_mol,input_file_ion,i)
        end=time.time()
        time=end-start
        print(time)
        if time > 15:
            print("TIME OUT 15s, 100% reach! Full packing number is approximately "+str(i))
            break
    return i
...
def main(fileinput):    
    ...
    if input_file_zeo and input_file_mol:
        print(input_file_zeo,input_file_mol)

        half_fill_number=int(get_100_packing_number(input_file_zeo,input_file_mol,input_file_ion)-1)

        if os.path.exists("output.pdb"):
            os.remove("output.pdb")
        packmol_to_zeo(input_file_zeo,input_file_mol,input_file_ion,half_fill_number)
       ...
       #   print("TIME OUT 30s, 100% reach! Full packing number is approximately "+str(get_100_packing_number(input_file_zeo,input_file_mol)))
        print("TIME OUT 15s, 100% reach! Half Packing Number is "+str(half_fill_number))
        ...
        with open("add_ions_number", "a+") as f:
            f.truncate(0)
            f.writelines([str(half_fill_number*4)])
