...
def packmol_to_zeo(input_file_zeo,input_file_mol,number):
...
import numpy as np
def get_100_packing_number(input_file_zeo,input_file_mol):
    for i in np.arange(0,1000,2):
        import time
        start=time.time()
        packmol_to_zeo(input_file_zeo,input_file_mol,i)
        end=time.time()
        time=end-start
        print(time)
        if time > 15:
...
def main(fileinput):    
    ...
    if input_file_zeo and input_file_mol:
        ...
        half_fill_number=int(get_100_packing_number(input_file_zeo,input_file_mol)/2)+3

        if os.path.exists("output.pdb"):
            os.remove("output.pdb")
        packmol_to_zeo(input_file_zeo,input_file_mol,half_fill_number)
